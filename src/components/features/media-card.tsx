...
export function MediaCard({ media }: MediaCardProps) {
  const card = (
    <div
      className={`group relative flex flex-col overflow-hidden rounded-3xl border border-plum-100 bg-white/70 transition-all ${
        media.published
          ? "hover:-translate-y-1 hover:border-lavender-300 hover:shadow-xl hover:shadow-lavender-300/20"
          : "opacity-70"
      }`}
    >
      <div
        className={`flex h-44 items-center justify-center bg-gradient-to-br ${media.cardGradient} text-6xl`}
      >
